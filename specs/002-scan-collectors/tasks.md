...
- **Note on ID T043a**: This task was inserted mid-phase as an addition to Phase 8 and intentionally uses an alphabetic suffix to avoid renumbering existing task references (commits and history reference T043 and T043a by name). T050 is the next sequential new task.
...
- [X] T032 [US3] Implement `scan/src/main/kotlin/dev/zahaand/projectscan/scan/adapter/IjLinterAdapter.kt` — Maven: `MavenProject.findPlugin("org.apache.maven.plugins", "maven-checkstyle-plugin")` + `findPlugin("org.apache.maven.plugins", "maven-pmd-plugin")`; for each plugin, iterate `plugin.executions` and emit **one `LinterToolDescriptor` per distinct `configFilePath`** across plugin-level and per-execution configs (FR-008 / CHK015 — multiple configurations of the same tool are collected independently); `breaksBuild` from `failOnViolation` at execution level overriding plugin level (not `failsOnError` per CHK013); if a plugin has no executions, emit a single descriptor from the plugin-level config (unchanged behaviour); Gradle: scan task names for `checkstyleMain`/`checkstyleTest`/`pmdMain`/`pmdTest` in ExternalProjectDataCache; Gradle config-file lookup restricted to `config/pmd/` paths only — standalone `pmd.xml` and `ruleset.xml` at project root are excluded to avoid false-positive PMD classification (A1 fix); Gradle `breaksBuild = null` always per research.md R-005; Wiring of `linterConfigParsers` into `LinterCollector` happens in T041; this adapter is unaffected.
...
- [X] T050 Update `CHANGELOG.md` to record deliberate Sprint 2 model changes required by SC-006 and constitution §Governance: removed `packageOrganization` field (replaced by `rootPackages + packageSegments` — FR-017); widened `breaksBuild: Boolean → Boolean?` in `ActiveRule` (FR-009/FR-010); replaced `namingPattern: String?` with `namingSuffixes: List<String>` in `TestInfo` (FR-013); added `unknownTestDependencies: List<Dependency>` (FR-012); additive additions (`getJdkVersion()`, `jdkVersion`, `toolsWithUnresolvableConfig`) noted as non-breaking
...
- [X] T044 [P] Write `scan/src/test/kotlin/dev/zahaand/projectscan/scan/adapter/IjBuildSystemAdapterTest.kt` — IntelliJ Platform fixture: Maven-ized project → `getBuildSystem()` returns `MAVEN`; `getModuleLanguageLevels()` returns the configured source compatibility level
- [X] T045 [P] Write `scan/src/test/kotlin/dev/zahaand/projectscan/scan/adapter/IjDependencyAdapterTest.kt` — IntelliJ Platform fixture: Maven project with 2 declared dependencies → `getModuleDependencies()` returns exactly those 2 entries; no transitive entries appear
- [X] T046 [P] Write `scan/src/test/kotlin/dev/zahaand/projectscan/scan/adapter/IjStyleSourceAdapterTest.kt` — IntelliJ Platform fixture: project with `.editorconfig` and `.idea/codeStyles/` → `findStyleSources()` returns both; project with neither → empty list
- [X] T047 [P] Write `scan/src/test/kotlin/dev/zahaand/projectscan/scan/adapter/IjLinterAdapterTest.kt` — IntelliJ Platform fixture (Maven): project with `maven-checkstyle-plugin` in `<plugins>` → tool recorded as applied with `breaksBuild` from `failOnViolation` (not `failsOnError` — CHK013: `failsOnError` tests Checkstyle's internal error handling, not build failure on rule violations); plugin in `<pluginManagement>` only → not applied; **multi-execution scenario**: project with two Checkstyle plugin executions each having a distinct `<configLocation>` → adapter emits two `LinterToolDescriptor` entries, each carrying its own config path and `failOnViolation` value; Gradle adapter: smoke-level compilation check only (Gradle TAPI import not feasible in light fixture)
- [X] T048 [P] Write `scan/src/test/kotlin/dev/zahaand/projectscan/scan/adapter/IjTestInfoAdapterTest.kt` — IntelliJ Platform fixture: module with test source root → `getTestSourceRoots()` returns project-relative path; `getCoverageThreshold()` returns correct value from Maven jacoco plugin configuration; `getTestClassNames()` returns file names from test source root
- [X] T049 [P] Write `scan/src/test/kotlin/dev/zahaand/projectscan/scan/adapter/IjModuleStructureAdapterTest.kt` — IntelliJ Platform fixture: two-module project → `getModules()` returns both; `getPackageTree()` returns correct root packages and second-level segments in dotted notation
