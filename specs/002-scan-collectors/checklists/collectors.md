...
- [ ] CHK001 — Does the spec define behavior when a dependency's resolved version cannot be
...
- [ ] CHK002 — Are `jdkVersion` and `languageLevel` defined as distinct concepts with separate
  collection requirements, or does "JDK / language level" in FR-004 refer to a single value?
  [Clarity, FR-004, Ambiguity]

- [ ] CHK003 — Is the fallback defined when all modules lack an explicit language level (all
  inherit the project default)? Is the project-level default always available? [Completeness,
  FR-005, Gap]

- [ ] CHK004 — Does the spec define how non-semver version strings (e.g., `1.0.0.RELEASE`,
  `1.0-SNAPSHOT`, `2.0.0.Final`) are ordered when applying the max-version aggregation rule?
  [Clarity, FR-003, Ambiguity]

- [ ] CHK005 — The Assumptions section states "scope is recorded as a fact, not used as a
...
- [ ] CHK006 — Is the reference point for "project-relative file path" (FR-006) explicitly
  defined? (Relative to project root? VCS root? Module root?) [Clarity, FR-006, Ambiguity]

- [ ] CHK007 — Does the spec define the file name patterns or discovery criteria for each
...
- [ ] CHK008 — Spotless is a build plugin without a dedicated standalone config file; its
...
- [ ] CHK009 — Is behavior defined for projects with multiple `.editorconfig` files (root-level
  and nested per-directory)? Are all of them collected or only the root? [Completeness, FR-006,
  Gap]

- [ ] CHK010 — Does the spec precisely define which file(s) under `.idea/` constitute "IDE style
...
- [ ] CHK011 — Does the spec explicitly enumerate the complete set of linter tools for which rule
...
- [ ] CHK012 — Is the default severity defined for a linter rule that declares no explicit
...
- [ ] CHK013 — Does the spec define how "applied" linter state is determined independently for
...
- [ ] CHK014 — Is behavior defined when a Checkstyle config references or imports an external
...
- [ ] CHK015 — Is behavior defined when multiple configurations of the same linter tool coexist
...
- [ ] CHK016 — The spec states the build-failure flag "lives at tool level" and is "denormalized
...
- [ ] CHK017 — Does the spec enumerate the full closed list of recognised test framework
...
- [ ] CHK018 — Is "test scope" explicitly defined for both Maven (which Maven scopes qualify:
...
- [ ] CHK019 — Does FR-013 define the exact format and content of "test source directory
...
- [ ] CHK020 — Is the numeric unit for the coverage threshold value explicitly specified
...
- [ ] CHK021 — Is behavior defined when JaCoCo is applied for report generation only (no
  threshold/check rule configured)? Should `coverageThreshold` be `null` or absent in that
  case? [Completeness, FR-014, Gap]

- [ ] CHK022 — Is the value recorded as `TestFramework.version` explicitly defined (dependency's
...
- [ ] CHK023 — Is "module name" explicitly defined — does it refer to the build system module
  identifier (e.g., Gradle `:core`, Maven `core`) or the IntelliJ module display name, which
  may differ? [Clarity, FR-015, Ambiguity]

- [ ] CHK024 — Is the string format for package segments explicitly defined? The examples use
  dotted Java package notation (`com.example.web`) but path notation (`com/example/web`) is
  not ruled out. [Clarity, FR-016, Ambiguity]

- [ ] CHK025 — The edge cases section lists "What if two modules declare an identical
...
- [ ] CHK026 — Does the spec name the new raw-material field replacing `packageOrganization`?
...
- [ ] CHK027 — Is behavior defined for a fully source-less project (every module has no source
...
- [ ] CHK028 — Is the boundary between `Ok(populated data)` and `Empty` defined per section?
...
- [ ] CHK029 — The Key Entities description of `ScanResult` mentions only two states
...
- [ ] CHK030 — Does the spec provide concrete per-section examples distinguishing "no data
  found" (Empty) from "data source unresolvable" (Error) for each of the five sections?
  [Clarity, FR-018, Ambiguity]

- [ ] CHK031 — Is behavior defined when a partial error occurs within a single collector —
...
- [ ] CHK032 — SC-006 states "no breaking changes to meaning of existing model fields," yet
...
- [ ] CHK033 — Are acceptance scenarios provided for Gradle projects (not just Maven) for the
...
- [ ] CHK034 — Is SC-002 ("100% of declared dependency coordinates appear") testable via unit
...
- [ ] CHK035 — Are the linter collector requirements (FR-008–FR-011) specific enough to write
