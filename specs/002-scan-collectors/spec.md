...
- **FR-020**: The scan layer MUST NOT text-parse build files (pom.xml, build.gradle, build.gradle.kts, settings files, version catalogs) to extract project facts. Linter config XML files (e.g., `checkstyle.xml`, `pmd.xml`) are not build files and MAY be parsed. Spotless configured only inline in the Gradle build file generates no `StyleSource` record, as detecting it would require build-file parsing.
