...
- [X] T017 [US1] Add `fun renderInvertedTechStack(stack: InvertedTechStack, buildSystem: BuildSystem?, jdkVersion: String?, languageLevel: String?): String` to `shared/src/main/kotlin/dev/zahaand/projectscan/shared/OutputFormatters.kt`: render preamble (Build System, JDK Version, Language Level); uniform entries as `- coordinate:version [N modules]`; multi-version entries as coordinate header followed by indented `- version  aggregator: module1, module2` lines; two sentinel branches: (a) return `"not detected"` when `stack.entries.isEmpty()` AND all preamble values are null; (b) if `stack.entries.isEmpty()` but at least one preamble value is non-null, render the preamble lines only and omit the `"not detected"` sentinel — partial-preamble branch implemented under F2, covered by the F2 fix test in `OutputFormattersTest.kt` (depends on T015)
...
- [X] T034 [P] Run `./gradlew build` from repository root and fix any remaining compilation errors not resolved in story phases (build was green at Phase 5 sign-off; re-confirmed green after Phase 6 F2/F6 changes)
- [X] T035 Verify SC-005 and SC-004: (a) AUTOMATED — `SC-005 byte-identical` test in `ScanResultRendererSmokeTest.kt` asserts byte-identical Tech Stack content between renderer and prompt generator using `assertEquals` against `renderInvertedTechStack` directly; (b) SC-004 confirmed by code inspection — no standalone "Discrepancies" block exists; discrepancy info is embedded inline in multi-version entries. MANUAL portion: live-IDE byte-identical confirmation against real monorepo output.
- [X] T036 [P] Verify SC-002: AUTOMATED — `SC-002 - known transitive-only artifacts are absent from direct-only input stack` test added to `OutputFormattersTest.kt`; asserts asm, objenesis, listenablefuture, failureaccess, j2objc-annotations, checker-qual, aopalliance, paranamer are absent when stack is built from a direct-only module set. MANUAL portion: live-IDE confirmation against reference Maven monorepo (SC-002 coverage is in the adapter layer, which cannot be unit-tested without IntelliJ fixtures).
- [ ] T037 Verify SC-001: MANUAL — scan the 130-module Maven monorepo and confirm Tech Stack output is ≤ 40 lines (preamble lines excluded from count). Cannot be automated without a running IntelliJ instance and the reference project.
- [X] T038 Record the Sprint 9 constitution-amendment obligation for the `shared` component: constitution §Project Structure must add `shared` to the component table with its dependency rules (`prompt` and `ui` MAY depend on `shared`; `scan` MUST NOT depend on `shared`). This is a tracking-only marker; the amendment lands in the Sprint 9 constitution package. No constitution edit in Sprint 7 (FR-N3).
- [ ] T040 Verify SC-006: MANUAL — scan any Maven project; confirm no "Project Structure" section and no `rootPackages`/`secondLevelSegments` value appears anywhere in the prompt or UI tool window output. Cannot be automated without a running IntelliJ instance.
- [ ] T041 Verify SC-003: MANUAL — scan any project; confirm the Testing section has no "Frameworks:" header and no framework name entries, and that coverage threshold, test source roots, and naming pattern fields are present. Cannot be automated without a running IntelliJ instance.
- [X] T042 Verify SC-007: AUTOMATED — `buildInvertedTechStack - SC-007 dep with resolved-from-parent version appears with that version` test in `OutputFormattersTest.kt` is green. MANUAL portion: live-IDE confirmation against a Maven project with parent-managed versions.
