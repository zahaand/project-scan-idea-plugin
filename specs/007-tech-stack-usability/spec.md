...
- **FR-007**: The shared module MUST build an inverted Tech Stack representation covering **all** direct external dependencies (scope filter: none — compile, runtime, test, and provided scopes are all included; system scope is excluded; the set is otherwise bounded by FR-003's declared-direct definition): a mapping from technology coordinate (`groupId:artifactId`) to an ordered list of (version, carrier-modules) pairs, sorted alphabetically by `groupId:artifactId`. A dependency is **external** when its coordinate is NOT an internal project module: neither its `artifactId` nor its `groupId:artifactId` matches any entry in `internalModuleNames`. `internalModuleNames` is derived from `StructureInfo.modules` as `modules.map { it.name }.toSet()` and is passed by both consumers (`PromptGenerator` and `ScanResultRenderer`) into `buildInvertedTechStack`. Test-framework coordinates (JUnit, Mockito, Testcontainers, AssertJ, etc.) appear in Tech Stack only; they are NOT duplicated in the Testing section.
...
- **SC-002**: No transitive-only artifact (`org.ow2.asm` groupId — covers all asm coordinates, objenesis, listenablefuture, checker-qual, aopalliance, paranamer, failureaccess, j2objc-annotations, or similar) appears in the Tech Stack or Testing output for any Maven project.
