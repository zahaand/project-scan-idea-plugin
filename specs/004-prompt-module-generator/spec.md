...
- **SC-005**: When all five scan sections are `SectionResult.Empty`, the generated prompt contains baseline-sourced Core Principles content and explicit `"not detected"` markers (per FR-008: `SectionResult.Empty` → `"not detected"`, `SectionResult.Error` → `"not available"`) in all scan-dependent blocks — verified by unit test inspection of the rendered string. The two markers are not interchangeable.
