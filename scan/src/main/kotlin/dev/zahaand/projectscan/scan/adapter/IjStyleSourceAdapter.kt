...
class IjStyleSourceAdapter(private val project: Project) : StyleSourcePort {
    override fun findStyleSources(): List<StyleSource> {
        ...
        val base = basePath?.let { LocalFileSystem.getInstance().findFileByPath(it) }
        return if (basePath == null || base == null) {
            emptyList()
        } else {
            buildList {
                ...
                addAll(
                    byNamePattern(base, "checkstyle", StyleSourceType.CHECKSTYLE, maxDepth = 2) {
                ...
                addAll(
                    byNamePattern(base, "pmd", StyleSourceType.PMD, maxDepth = 2) {
    ...
    private fun walkDepth(
        dir: VirtualFile,
        maxDepth: Int,
        depth: Int = 1,
        action: (VirtualFile) -> Unit,
    ) {
    ...
    private fun toRelative(
        base: VirtualFile,
        file: VirtualFile,
    ): String = file.path.removePrefix(base.path).trimStart('/')
}
